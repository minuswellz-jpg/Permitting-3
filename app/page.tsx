...
type IntakeData = {
  ...
  project_details: {
    ...
    same_footprint: boolean | null;
    new_piles: boolean | null;
    dock_length_ft: number | null;
    dock_width_ft: number | null;
    overwater_sqft: number | null;
  };
  permit_flags: {
    shoreline_review_likely: boolean | null;
    hpa_required_likely: boolean | null;
    usace_review_likely: boolean | null;
    dnr_aquatic_lands_check: boolean | null;
  };
  documents_received: {
    site_plan: boolean;
    photos: boolean;
    survey: boolean;
    parcel_info: boolean;
    ownership_docs: boolean;
    dock_design_drawings: boolean;
  };
  missing_documents: string[];
  agency_routes: string[];
  status: string;
  notes: string;
};

type Message = { role: 'assistant' | 'user'; text: string };
type Question = { key: string; label: string; type: 'text' | 'textarea' | 'number' | 'boolean' };
...
function computeFlags(data: IntakeData) {
...
function computeMissingDocuments(data: IntakeData) {
  return Object.entries(data.documents_received)
    .filter(([, value]) => value === false)
    .map(([key]) => docLabels[key]);
}

function computeAgencyRoutes(flags: IntakeData['permit_flags']) {
  const routes: string[] = [];
...
export default function HomePage() {
  ...
  const handleAnswer = (rawValue: string | boolean) => {
    const transcript = [...messages, { role: 'user' as const, text: typeof rawValue === 'boolean' ? (rawValue ? 'Yes' : 'No') : rawValue || '—' }];
    const q = questions[step];
    let value: string | boolean | number | null = rawValue as any;
